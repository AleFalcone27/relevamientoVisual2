...
import { addOutline, createOutline, cameraOutline, imageOutline, trashOutline, pieChartOutline, albumsOutline, trendingUpOutline, logOut } from 'ionicons/icons';
...
export class CosasFeasPage implements OnInit {
  ...
  extractImageInfo(url: string): { email: string; timestamp: string } | null {
    const match = url.match(/cosasFeas\/(.+?)\.jpeg$/);
    if (match && match[1]) {
